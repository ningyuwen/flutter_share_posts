...
/// [SearchDelegateMine.close] call. If the search page is closed with the system
/// back button, it returns null.
///
/// A given [SearchDelegateMine] can only be associated with one active [showSearch]
/// call. Call [SearchDelegateMine.close] before re-using the same delegate instance
...
/// The animation provided by [SearchDelegateMine.transitionAnimation] can be used
...
/// See also:
///
///  * [SearchDelegateMine] to define the content of the search page.
Future<T> showSearchMine<T>({
  @required BuildContext context,
  @required SearchDelegateMine<T> delegate,
  String query = '',
}) {
...
/// query text field can be customized via [SearchDelegateMine.leading] and
/// [SearchDelegateMine.actions].
///
/// The body below the [AppBar] can either show suggested queries (returned by
/// [SearchDelegateMine.buildSuggestions]) or - once the user submits a search  - the
/// results of the search as returned by [SearchDelegateMine.buildResults].
///
/// [SearchDelegateMine.query] always contains the current query entered by the user
/// and should be used to build the suggestions and results.
///
/// The results can be brought on screen by calling [SearchDelegateMine.showResults]
/// and you can go back to showing the suggestions by calling
/// [SearchDelegateMine.showSuggestions].
///
/// Once the user has selected a search result, [SearchDelegateMine.close] should be
...
/// call. Call [SearchDelegateMine.close] before re-using the same delegate instance
/// for another [showSearch] call.
abstract class SearchDelegateMine<T> {
  ...
  final ValueNotifier<_SearchBody> _currentBodyNotifier = ValueNotifier<
      _SearchBody>(null);
...
enum _SearchBody {
  ...
  ///
  /// The suggested queries are generated by [SearchDelegateMine.buildSuggestions].
  suggestions,

  /// Search results are currently shown in the body.
  ///
  /// The search results are generated by [SearchDelegateMine.buildResults].
  results,
}
...
class _SearchPageRoute<T> extends PageRoute<T> {
  ...
  final SearchDelegateMine<T> delegate;
...
class _SearchPageState<T> extends State<_SearchPage<T>> {
  ...
  Widget build(BuildContext context) {
    ...
    final String searchFieldLabel = MaterialLocalizations
        .of(context)
        .searchFieldLabel;
    Widget body;
